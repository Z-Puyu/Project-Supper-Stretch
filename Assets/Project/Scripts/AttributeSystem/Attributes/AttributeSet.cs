...
public class AttributeSet : MonoBehaviour, IPersistent, IVisitable<AttributeSet> {
    ...
    private void UpdateAttribute(in Attribute newData) {
        ...
        this.Attributes[newData.Type] = updated;
        this.PostAttributeUpdate(in updated);
        this.OnAttributeChanged.Invoke(this, old, updated);
        Debug.Log($"{updated.Type}: {old.CurrentValue} -> {updated.CurrentValue}");
    }
