...
public readonly record struct Modifier(ModifierKey Key, float Value, int Duration = 0) : IPresentable<ModifierLocalisationMapping> {
    public ModifierType Type => this.Key.Type;
    public string Target => this.Key.Target;
    
    private string ValueText => this.Type == ModifierType.Multiplier 
            ? $"{this.Value:+#;-#;+#}%" 
            : $"{this.Value:+#;-#;+#}";
    
    private string TargetName => this.Target.Split('.')[^1];
    ...
    public string FormatAsText(ModifierLocalisationMapping? reference = null) {
        string desc = !reference
                ? $"{this.ValueText} {this.TargetName}"
                : reference.Map(this).Replace("{value}", this.ValueText);
        return this.Duration switch {
            > 0 => $"{desc} per second, for {this.Duration} seconds",
            0 => desc,
            < 0 => $"{desc} per second"
        };
    }
