...
namespace GDS.Core {
    public static class InventoryExtensions {
        ...
        public static Slot SetItemData(this Slot slot, ItemData itemData) => slot with { Item = slot.Item with { ItemData = itemData } };
        ...
        public static bool SetItem(this Bag bag, Slot slot, Item item) {
            return (bag, slot) switch {
                ...
                (SetBag b, SetSlot s) => b.SetItem(s.Key, item),
                _ => false
            };
        }
        ...
        public static bool MoveItem(this Bag fromBag, Bag toBag, Item item) {
            Debug.Log($"Should move from {fromBag.Id} to {toBag.Id}");
        ...
        static bool MoveItem(this ListBag fromBag, ListBag toBag, Item item) {
            var success = toBag.Accepts(item) && AddItem(toBag, item) && RemoveItem(fromBag, item);
            if (success) { fromBag.Data.Notify(); toBag.Data.Notify(); }
            return success;
        }
