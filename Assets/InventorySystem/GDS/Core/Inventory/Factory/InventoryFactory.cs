...
namespace GDS.Core {
    ...
    public static class InventoryFactory {
        ...
        public static T CreateSetBag<T>(string id, SetSlot[] slots) where T : SetBag {
            var slotsDict = slots.ToDictionary(slot => slot.Key, slot => slot);
            var bag = (T)Activator.CreateInstance(typeof(T), id, new Observable<Dictionary<string, SetSlot>>(slotsDict));
            return bag;
        }
