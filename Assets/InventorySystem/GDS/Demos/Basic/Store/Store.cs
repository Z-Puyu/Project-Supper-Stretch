...
namespace GDS.Basic {
    ...
    public class Store {
        public Store() {
            ...
            Bus.Subscribe<SavePlayerData>(e => OnSavePlayerData(e as SavePlayerData));
            Bus.Subscribe<LoadPlayerData>(e => OnLoadPlayerData(e as LoadPlayerData));
        ...
        public readonly SetBag Equipment = CreateSetBag<SetBag>("equipment", Factory.CreateEquipmentSlots());
        ...
        void OnBuyItem(BuyItemEvent e) {
            ...
            else Bus.Publish(ResultEvent.Fail);
        }
        ...
        void OnToggleInventory(ToggleInventoryEvent e) {
            ...
            Debug.Log($"is inventory open? {IsInventoryOpen.Value}");
        ...
        void OnSavePlayerData(SavePlayerData e) {
            LogEvent(e);

            PlayerData playerData = new("Jim", 1, Equipment, Main);
            SaveSystem.Save(playerData);
        }

        void OnLoadPlayerData(LoadPlayerData e) {
            LogEvent(e);

            PlayerData playerData = SaveSystem.Load();
            if (playerData == null) return;
            Equipment.SetState(
                playerData.equipment.Select(item => (item.slot, CreateItem(Bases.Get(item.baseId), item.rarity))).ToArray()
            );

            Main.SetState(
                playerData.inventory.Select(item => CreateItem(Bases.Get(item.baseId), item.rarity, item.quant)).ToArray()
            );
