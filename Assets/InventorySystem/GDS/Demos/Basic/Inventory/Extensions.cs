...
namespace GDS.Basic {
    public static class Extensions {
        ...
        public static CustomEvent UnstackItem(Bag bag, Item item, Slot slot, Observable<Item> dragged) => bag.UnstackItem(item, slot, dragged) ? new PickItemSuccess(item) : ResultEvent.Fail;
        public static (Item, CustomEvent) UnstackDraggedItem(Bag bag, Slot slot, Observable<Item> dragged) => InventoryExtensions.UnstackDraggedItem(bag, slot, dragged) ? (Item.NoItem, new PlaceItemSuccess(dragged.Value)) : (Item.NoItem, ResultEvent.Fail);
        ...
        static CustomEvent CraftAndMoveItem(this CraftingBench bag, Bag toBag, Item item, CraftingOutcomeSlot slot) {
            ...
            return toBag.AddItem(item.Clone()) ? new CraftItemSuccess(item) : ResultEvent.Fail;
        }
        ...
        static (Item, CustomEvent) PickItem(this Bag bag, Item item, Slot slot) {
            ...
            return success ? (replaced, new PickItemSuccess(item)) : (replaced, ResultEvent.Fail);
        }
        ...
        static (bool, Item, CustomEvent) PlaceItem(this Bag bag, Item item, Slot slot) {
            ...
            return success ? (success, replaced, new PlaceItemSuccess(item)) : (success, replaced, ResultEvent.Fail);
        }

        static (Item, CustomEvent) BuyAndCloneItem(this Vendor bag, Item item) => (item.Clone(), new BuyItemSuccess(item));
        static (Item, CustomEvent) BuyItem(this Vendor bag, Item item) => bag.RemoveItem(item) ? (item, new BuyItemSuccess(item)) : (item, ResultEvent.Fail);

        static CustomEvent BuyAndMoveItem(this Vendor bag, Bag toBag, Item item) => InventoryExtensions.MoveItem(bag, toBag, item) ? new BuyItemSuccess(item) : ResultEvent.Fail;
        static CustomEvent BuyAndCloneItem(this Vendor bag, Bag toBag, Item item) => toBag.AddItem(item.Clone()) ? new BuyItemSuccess(item) : ResultEvent.Fail;
        static CustomEvent SellItem(this Main bag, Item item) => bag.RemoveItem(item) ? new SellItemSuccess(item) : ResultEvent.Fail;
        static CustomEvent MoveItem(this Bag bag, Bag toBag, Item item) => InventoryExtensions.MoveItem(bag, toBag, item) ? new MoveItemSuccess(item) : ResultEvent.Fail;
