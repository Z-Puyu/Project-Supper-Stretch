...
namespace Flexalon.Runtime.Core
{
...
#endif
    {
        ...
        {
            get { return this._updateInEditMode; }
            set { this._updateInEditMode = value; }
        }
        ...
        {
            get { return this._updateInPlayMode; }
            set { this._updateInPlayMode = value; }
        }
        ...
        {
            get { return this._skipInactiveObjects; }
            set { this._skipInactiveObjects = value; }
        }
        ...
        /// </summary>
        public IReadOnlyCollection<FlexalonNode> Nodes => this._nodes;
        ...
        {
            get => Flexalon._recordFrameChanges && !Flexalon._undoRedo;
            set => Flexalon._recordFrameChanges = value;
        }
        ...
        {
            return Flexalon._instance;
        }

        public static Flexalon GetOrCreate()
        {
            Flexalon.TryGetOrCreate(out _);
            return Flexalon._instance;
        }
        ...
        {
            bool created = false;
            if (!Flexalon._instance)
            {
                #if UNITY_2023_1_OR_NEWER
                    Flexalon._instance = Object.FindFirstObjectByType<Flexalon>();
                #else
                ...
                if (!Flexalon._instance)
                {
                    ...
                    #endif
                    Flexalon._instance = Flexalon.AddComponent<Flexalon>(FlexalonGO);
                    created = true;
                }
                else
                {
        ...
        {
            if (Flexalon._instance != null && Flexalon._instance && Flexalon._instance._gameObjects.TryGetValue(go, out var node))
            {
                return node;
            }
        ...
        public static FlexalonNode GetOrCreateNode(GameObject go)
        {
            ...
            Flexalon.GetOrCreate();

            if (!Flexalon._instance._gameObjects.TryGetValue(go, out var node))
            {
                node = Flexalon._instance.CreateNode();
                node._gameObject = go;
                node.RefreshResult();
                ...
                Flexalon._instance._gameObjects.Add(go, node);
            }
        ...
        public static InputProvider GetInputProvider()
        {
            ...
            if (Flexalon._instance)
            {
                if (Flexalon._instance._input == null)
                {
                    if (Flexalon._instance._inputProvider)
                    {
                        Flexalon._instance._input = Flexalon._instance._inputProvider.GetComponent<InputProvider>();
                    }

                    if (Flexalon._instance._input == null)
                    {
                        Flexalon._instance._input = new FlexalonMouseInputProvider();
                    }
                }

                return Flexalon._instance._input;
            }
        ...
        {
            return (T)Flexalon.AddComponent(go, typeof(T));
        }
        ...
        {
            var node = new Node();
            node._transformUpdater = this._defaultTransformUpdater;
        ...
        {
            if (Flexalon._instance != null && Flexalon._instance._gameObjects.TryGetValue(go, out var node))
            {
                Flexalon._instance._gameObjects.Remove(go);
                node.Detach();
        ...
        void LateUpdate()
        {
            ...
            if (!Application.isPlaying && this._updateInEditMode)
            {
                this.UpdateDirtyNodes();
            }

            Flexalon._undoRedo = false;
            Flexalon.RecordFrameChanges = false;
        }
        ...
        public void UpdateDirtyNodes()
        {
            ...
            foreach (var kv in this._gameObjects)
            {
                var go = kv.Key;
                var node = kv.Value;
                if (!go)
                {
                    this._destroyed.Add(go);
                }
                else
                {
            ...
            foreach (var go in this._destroyed)
            {
                this.DestroyNode(go);
            }
        ...
        void OnEnable()
        {
            if (Flexalon._instance == this)
            {
                UnityEditor.Undo.undoRedoPerformed += this.OnUndoRedo;
            }
        }
#endif
        ...
        void OnDestroy()
        {
            if (Flexalon._instance == this)
            {
                ...
                Flexalon._instance = null;
            }
        }

        private void OnUndoRedo()
        {
            Flexalon._undoRedo = true;
        }
        ...
        private void Compute(Node node)
        {
            ...
            if (node.HasResult)
            {
                this.ComputeTransforms(node);
                this.UpdateTransforms(node);
                this.ComputeDependents(node);
            }
        }
        ...
        private void MeasureRoot(Node node)
        {
            ...
            this.Measure(node, min, max, true);
        }
        ...
        private void MeasureChild(Node node, bool includeChildren = true)
        {
            ...
            this.Measure(node, min, max, includeChildren);
        }
        ...
        private void Measure(Node node, Vector3 min, Vector3 max, bool includeChildren = true)
        {
            ...
            var size = Flexalon.MeasureSize(node);
            this.MeasureAdapterSize(node, Vector3.zero, size, min, max);

            if (includeChildren && node.Method != null)
            {
                this.MeasureLayout(node, min, max);
            }
        ...
        private void MeasureLayout(Node node, Vector3 min, Vector3 max)
        {
            ...
            foreach (var child in node._children)
            {
                ...
                if (Flexalon.AnyAxisIsFill(child))
                {
                    this.MeasureChild(child, false);
                }
            ...
            var childAvailableSize = Flexalon.GetChildAvailableSize(node);
            ...
            this.MeasureAdapterSize(node, layoutBounds.center, layoutBounds.size + node.Padding.Size, min, max);

            // Measure any children that depend on our size
            bool anyChildSizeChanged = false;
            foreach (var child in node._children)
            {
                if (Flexalon.AnyAxisIsFill(child) || child.IsShrunk())
                {
                    var previousSize = child.GetArrangeSize();

                    this.MeasureChild(child, layoutBounds.size, true);
            ...
            if (anyChildSizeChanged)
            {
                ...
                foreach (var child in node._children)
                {
                    if (Flexalon.AnyFillOrShrinkSizeChanged(child))
                    {
                        this.MeasureChild(child, layoutBounds.size, true);
                        child._dirty = true;
                    }
        ...
        private void ComputeTransforms(Node node)
        {
            if (node.HasSizeUpdate)
            {
                ...
                this.ComputeScale(node);
                this.ComputeRectSize(node);
        ...
        {
            return Flexalon.AxisIsFill(child, 0) || Flexalon.AxisIsFill(child, 1) || Flexalon.AxisIsFill(child, 2);
        }
        ...
        {
            return Flexalon.AxisIsFill(child, axis) && child.FillSizeChanged[axis];
        }

        private static bool AnyFillSizeChanged(Node child)
        {
            return Flexalon.FillSizeChanged(child, 0) ||
                Flexalon.FillSizeChanged(child, 1) ||
                Flexalon.FillSizeChanged(child, 2);
        }
        ...
        {
            return Flexalon.ShrinkSizeChanged(child, 0) ||
                Flexalon.ShrinkSizeChanged(child, 1) ||
                Flexalon.ShrinkSizeChanged(child, 2);
        }

        private static bool AnyFillOrShrinkSizeChanged(Node child)
        {
            return Flexalon.AnyFillSizeChanged(child) || Flexalon.AnyShrinkSizeChanged(child);
        }
        ...
        public void LayoutComplete() => this.UpdateDirtyNodes();
        ...
        private class Node : FlexalonNode
        {
            ...
            public Layout Method { get => this._method; set => this._method = value; }
            public Constraint _constraint;
            public Constraint Constraint => this._constraint;
            private Adapter _adapter = null;
            public Adapter Adapter => (this._adapter == null) ? this._adapter = new DefaultAdapter(this.GameObject, this) : this._adapter;
            public bool _customAdapter = false;
            public FlexalonResult _result;
            public FlexalonResult Result => this._result;
            public FlexalonObject _flexalonObject;
            public FlexalonObject FlexalonObject => this._flexalonObject;
            public Vector3 Size => this.HasFlexalonObject ? this._flexalonObject.Size : Vector3.one;
            public Vector3 SizeOfParent => this.HasFlexalonObject ? this._flexalonObject.SizeOfParent : Vector3.one;
            public Vector3 Offset => this.HasFlexalonObject ? this._flexalonObject.Offset : Vector3.zero;
            public Vector3 Scale => this.HasFlexalonObject ? this._flexalonObject.Scale : Vector3.one;
            public Quaternion Rotation => this.HasFlexalonObject ? this._flexalonObject.Rotation : Quaternion.identity;
            public Directions Margin => this.HasFlexalonObject ? this._flexalonObject.Margin : Directions.zero;
            public Directions Padding => this.HasFlexalonObject ? this._flexalonObject.Padding : Directions.zero;
            public Node _dependency;
            public FlexalonNode Dependency => this._dependency;
            public bool HasDependents => this._dependents != null && this._dependents.Count > 0;
            public List<Node> _dependents;
            public TransformUpdater _transformUpdater;
            public List<FlexalonModifier> _modifiers = null;
            public IReadOnlyList<FlexalonModifier> Modifiers => this._modifiers;
            public event System.Action<FlexalonNode> ResultChanged;
            public bool IsDragging { get; set; }
            private bool SkipInactive => Flexalon._instance._skipInactiveObjects && !this._gameObject.activeInHierarchy;
            public bool SkipLayout => this.SkipInactive || (this.HasFlexalonObject ? this._flexalonObject.SkipLayout : false);
            private bool _hasFlexalonObject;
            public bool HasFlexalonObject => this._hasFlexalonObject;
            public bool[] FillSizeChanged = new bool[3];
            public bool[] ShrinkSizeChanged = new bool[3];

            public void SetFillSize(Vector3 fillSize)
            {
                this.SetFillSize(0, fillSize.x);
                this.SetFillSize(1, fillSize.y);
                this.SetFillSize(2, fillSize.z);
            }

            public void SetFillSize(int axis, float size)
            {
                FlexalonLog.Log("SetFillSize", this, axis, size);
                this.FillSizeChanged[axis] = this.FillSizeChanged[axis] || (this._result.FillSize[axis] != size);
                this._result.FillSize[axis] = size;
            }

            public void ResetFillShrinkChanged()
            {
                this.FillSizeChanged[0] = this.FillSizeChanged[1] = this.FillSizeChanged[2] = false;
                this.ShrinkSizeChanged[0] = this.ShrinkSizeChanged[1] = this.ShrinkSizeChanged[2] = false;
            }

            public void ResetShrinkFillSize()
            {
                this._result.ShrinkSize = Math.MaxVector;
                this._result.FillSize = Vector3.zero;
            }

            public void SetShrinkSize(int axis, float size)
            {
                FlexalonLog.Log("SetShrinkSize", this, axis, size);
                this.ShrinkSizeChanged[axis] = this.ShrinkSizeChanged[axis] || (this._result.ShrinkSize[axis] != size);
                this._result.ShrinkSize[axis] = size;
            }
            ...
            public void SetShrinkFillSize(Vector3 childSize, Vector3 layoutSize, bool includesSizeOfParent)
            {
                for (int axis = 0; axis < 3; axis++)
                {
                    this.SetShrinkFillSize(axis, childSize[axis], layoutSize[axis], includesSizeOfParent);
                }
            }

            public void SetShrinkFillSize(int axis, float childSize, float layoutSize, bool includesSizeOfParent)
            {
                if (Flexalon.AxisIsFill(this, axis))
                {
                    var fillSize = includesSizeOfParent ?
                        (this.SizeOfParent[axis] > 0 ? childSize / this.SizeOfParent[axis] : 0) :
                        childSize;
                    this.SetFillSize(axis, fillSize);
                }

                if (this.GetMinSizeType(axis) != MinMaxSizeType.None)
                {
                    var measureSize = this.GetMeasureSize(axis, layoutSize);
                    if (measureSize > childSize)
                    {
                        this.SetShrinkSize(axis, Mathf.Max(this.GetMinSize(axis, layoutSize), childSize));
                    }
                }
            }
            ...
            {
                return this._result.ShrinkSize != Math.MaxVector;
            }

            public bool CanShrink(int axis)
            {
                return this.GetSizeType(axis) != SizeType.Fill && this.GetMinSizeType(axis) != MinMaxSizeType.None;
            }

            public void UpdateRootFillSize()
            {
                var newSize = this.GetRootFillSize();
                if (newSize != this._result.FillSize)
                {
                    FlexalonLog.Log("UpdateRootFillSize", this, newSize);
                    this.SetFillSize(newSize);
                    this.MarkDirty();
                }
            }
            ...
            {
                var fillSize = Flexalon._defaultSize;
                if (this.GameObject.transform.parent && this.GameObject.transform.parent is RectTransform parentRect)
                {
            ...
            {
                if (this.HasFlexalonObject)
                {
                    switch (axis)
                    {
                        case Axis.X: return this._flexalonObject.WidthType;
                        case Axis.Y: return this._flexalonObject.HeightType;
                        case Axis.Z: return this._flexalonObject.DepthType;
                    }
                }
            ...
            {
                return this.GetSizeType((Axis)axis);
            }

            public MinMaxSizeType GetMinSizeType(int axis)
            {
                return this.GetMinSizeType((Axis)axis);
            }
            ...
            public MinMaxSizeType GetMinSizeType(Axis axis)
            {
                if (this.HasFlexalonObject)
                {
                    switch (axis)
                    {
                        case Axis.X: return this._flexalonObject.MinWidthType;
                        case Axis.Y: return this._flexalonObject.MinHeightType;
                        case Axis.Z: return this._flexalonObject.MinDepthType;
                    }
                }
            ...
            {
                return this.GetMinSize((Axis)axis, parentLayoutSize);
            }

            public float GetMinSize(Axis axis, float parentLayoutSize)
            {
                return this.GetMinSize(axis, parentLayoutSize, true);
            }

            public float GetMinSize(Axis axis, float parentLayoutSize, bool withMargin)
            {
                var margin = withMargin ? this.Margin.Size[(int)axis] : 0;

                if (this.HasFlexalonObject)
                {
                    switch (this.GetMinSizeType(axis))
                    {
                        case MinMaxSizeType.None:
                            return margin;
                        case MinMaxSizeType.Fixed:
                            return Mathf.Max(this._flexalonObject.MinSize[(int)axis], 0) + margin;
                        case MinMaxSizeType.Fill:
                            return Mathf.Max(this._flexalonObject.MinSizeOfParent[(int)axis] * parentLayoutSize, 0);
                    }
                }
            ...
            {
                return this.GetMinSize(parentLayoutSize, true);
            }

            public Vector3 GetMinSize(Vector3 parentLayoutSize, bool withMargin)
            {
                return new Vector3(this.GetMinSize(Axis.X, parentLayoutSize.x, withMargin),
                    this.GetMinSize(Axis.Y, parentLayoutSize.y, withMargin),
                    this.GetMinSize(Axis.Z, parentLayoutSize.z, withMargin));
            }

            public MinMaxSizeType GetMaxSizeType(int axis)
            {
                return this.GetMaxSizeType((Axis)axis);
            }
            ...
            public MinMaxSizeType GetMaxSizeType(Axis axis)
            {
                if (this.HasFlexalonObject)
                {
                    switch (axis)
                    {
                        case Axis.X: return this._flexalonObject.MaxWidthType;
                        case Axis.Y: return this._flexalonObject.MaxHeightType;
                        case Axis.Z: return this._flexalonObject.MaxDepthType;
                    }
                }
            ...
            {
                return this.GetMaxSize(parentLayoutSize, true);
            }

            public float GetMaxSize(int axis, float parentLayoutSize)
            {
                return this.GetMaxSize((Axis)axis, parentLayoutSize);
            }

            public float GetMaxSize(Axis axis, float fillSize)
            {
                return this.GetMaxSize(axis, fillSize, true);
            }
            ...
            public float GetMaxSize(Axis axis, float parentLayoutSize, bool withMargin)
            {
                ...
                if (this.HasFlexalonObject)
                {
                    switch (this.GetMaxSizeType(axis))
                    {
                        case MinMaxSizeType.None:
                            return Math.MaxValue;
                        case MinMaxSizeType.Fixed:
                            return Mathf.Max(0, this._flexalonObject.MaxSize[(int)axis]) + margin;
                        case MinMaxSizeType.Fill:
                            return Mathf.Max(0, this._flexalonObject.MaxSizeOfParent[(int)axis] * parentLayoutSize);
                    }
                }
            ...
            {
                return new Vector3(this.GetMaxSize(Axis.X, parentLayoutSize.x, withMargin),
                    this.GetMaxSize(Axis.Y, parentLayoutSize.y, withMargin),
                    this.GetMaxSize(Axis.Z, parentLayoutSize.z, withMargin));
            }

            public void SetPositionResult(Vector3 position)
            {
                this.RecordResultUndo();
                this._result.LayoutPosition = position;
            ...
            {
                this.RecordResultUndo();
                this._result.LayoutRotation = quaternion;
                this.HasRotationUpdate = true;
                this.UpdateDependents = true;
            }
            ...
            {
                this.RecordResultUndo();
                this._result.ComponentScale = scale;
            }
            ...
            {
                updater = updater != null ? updater : Flexalon._instance?._defaultTransformUpdater;
                if (updater != this._transformUpdater)
                {
                    this._transformUpdater = updater;
                }
            }

            public void SetFlexalonObject(FlexalonObject obj)
            {
                this._hasFlexalonObject = obj != null;
                this._flexalonObject = obj;
            }
            ...
            public void MarkDirty()
            {
                ...
                if (this._dependency != null && !this._dependency.HasResult)
                {
                    this._dependency?.MarkDirty();
                }
            ...
            private void MarkDirtyDown()
            {
                ...
                {
                    foreach (var dep in this._dependents)
                    {
                        dep.MarkDirty();
                        dep.MarkDirtyDown();
                    }
            ...
            {
                this.InsertChild(child, this._children.Count);
            }
            ...
            public void InsertChild(FlexalonNode child, int index)
            {
                ...
                Flexalon._instance?._roots.Remove(childNode);
            }
            ...
            public void Detach()
            {
                if (this._parent != null)
                {
                    ...
                    if (Flexalon._instance != null && Flexalon._instance._gameObjects.ContainsKey(this.GameObject))
                    {
                        Flexalon._instance._roots.Add(this);
                    }
                }
            }
            ...
            public void DetachAllChildren()
            {
                ...
                {
                    this.Children[this.Children.Count - 1].Detach();
                }
            }

            public Vector3 GetMeasureSize(Vector3 layoutSize)
            {
                return new Vector3(this.GetMeasureSize(0, layoutSize.x),
                    this.GetMeasureSize(1, layoutSize.y),
                    this.GetMeasureSize(2, layoutSize.z));
            }

            public float GetMeasureSize(int axis, float layoutSize)
            {
                var size = this.GetSizeType(axis) == SizeType.Fill ? 0 : this._result.RotatedAndScaledBounds.size[axis] + this.Margin.Size[axis];
                return Mathf.Clamp(size, this.GetMinSize(axis, layoutSize), this.GetMaxSize(axis, layoutSize));
            }

            public Vector3 GetArrangeSize()
            {
                return this._result.RotatedAndScaledBounds.size + this.Margin.Size;
            }

            public Vector3 GetBoxScale()
            {
                bool shouldScale = this.Adapter.TryGetScale(this, out var _);
                if (!shouldScale)
                {
                    return Math.Abs(this.GameObject.transform.localScale);
                }
                else if (this.HasFlexalonObject)
                {
                    // FlexalonObject size/scale always applies, even without a layout.
                    return Math.Abs(this._flexalonObject.Scale);
                }
            ...
            public Quaternion GetBoxRotation()
            {
                ...
                if (this._parent != null || this._dependency != null)
                {
                    return this.HasFlexalonObject ? this._flexalonObject.Rotation : Quaternion.identity;
                }
                else
                {
            ...
            {
                Vector3 scale = includeLocalScale ? this.GetBoxScale() : Vector3.one;
                var node = this;
            ...
            {
                var pos = this._result.LayoutBounds.center;
                if (includePadding)
                {
                    pos -= this.Padding.Center;
                }

                pos.Scale(scale);
                pos = this.GameObject.transform.rotation * pos + this.GameObject.transform.position;
                return pos;
            }

            public void SetDependency(FlexalonNode node)
            {
                if (this._dependency != node)
                {
                    this._dependency?._dependents.Remove(this);

                    this._dependency = node as Node;

                    if (node != null)
                    {
                        if (this._dependency._dependents == null)
                        {
                            this._dependency._dependents = new List<Node>();
                        }

                        this._dependency._dependents.Add(this);
                    }
                }
            }
            ...
            public void ClearDependents()
            {
                ...
                {
                    while (this._dependents.Count > 0)
                    {
                        this._dependents[this._dependents.Count - 1].SetDependency(null);
                    }
                }
            }
            ...
            public void SetAdapter(Adapter adapter)
            {
                ...
                this._customAdapter = (this._adapter != null);
            }
            ...
            public void CheckDefaultAdapter()
            {
                ...
                {
                    if ((this.Adapter as DefaultAdapter).CheckComponent(this.GameObject, this))
                    {
                        this.MarkDirty();
                    }
                }
            ...
            {
                var bounds = Math.ScaleBounds(this._result.LayoutBounds, this.GetBoxScale());
                bounds = Math.RotateBounds(bounds, this.GetBoxRotation());
                this.RecordResultUndo();
                this._result.RotatedAndScaledBounds = bounds;
                FlexalonLog.Log("Measure | RotatedAndScaledBounds", this, bounds);
                this.HasSizeUpdate = true;
                this.UpdateDependents = true;
            }

            public void RefreshResult()
            {
                this._result = this._gameObject.GetComponent<FlexalonResult>();
                this._hasResult = this._result != null;
                if (!this._hasResult)
                {
                    this._result = Flexalon.AddComponent<FlexalonResult>(this.GameObject);
                    this._dirty = true;
                }
            }
            ...
            {
                this._result.TransformPosition = this.GameObject.transform.localPosition;
                this._result.TransformRotation = this.GameObject.transform.localRotation;
                this._result.TransformScale = this.GameObject.transform.localScale;
                this._result.TargetPosition = this.GameObject.transform.localPosition;
                this._result.TargetRotation = this.GameObject.transform.localRotation;
                this._result.TargetScale = this.GameObject.transform.localScale;
            }

            public void RecordResultUndo()
            {
#if UNITY_EDITOR
                if (Flexalon.RecordFrameChanges && this._result != null)
                {
                    UnityEditor.Undo.RecordObject(this._result, "Result changed");
                    UnityEditor.PrefabUtility.RecordPrefabInstancePropertyModifications(this._result);
                }
#endif
            }
            ...
            public void AddModifier(FlexalonModifier modifier)
            {
                ...
                {
                    this._modifiers = new List<FlexalonModifier>();
                }

                this._modifiers.RemoveAll(m => m == modifier);
            ...
            public void DetectRectTransformChanged()
            {
                if (this.GameObject.transform is RectTransform rectTransform)
                {
                    ...
                    if (this.ReachedTargetRectSize && this._result.TransformRectSize != rectTransform.rect.size)
                    {
                        this.MarkDirty();
                    }
                }
