...
namespace Flexalon.Runtime.Adapters
{
    ...
    public class FlexalonColliderAdapter : FlexalonComponent, Adapter
    {
        ...
        public bool TryGetScale(FlexalonNode node, out Vector3 scale)
        {
            ...
            if (this._collider is BoxCollider boxCollider)
            {
                boxCollider.size = size;
            }
            else if (this._collider is SphereCollider sphereCollider)
            {
                sphereCollider.radius = Mathf.Min(size.x, size.y, size.z) * 0.5f;
            }
            else if (this._collider is CapsuleCollider capsuleCollider)
            {
            ...
            if (this._collider2D is BoxCollider2D boxCollider2D)
            {
                boxCollider2D.size = size;
            }
            else if (this._collider2D is CircleCollider2D circleCollider2D)
            {
                circleCollider2D.radius = Mathf.Min(size.x, size.y) * 0.5f;
            }
            else if (this._collider2D is CapsuleCollider2D capsuleCollider2D)
            {
