    ...
    public class CustomLayout : LayoutBase
    {
        ...
        private static void ShrinkFillChildren(IReadOnlyList<FlexalonNode> nodes, Vector3 usedSize, Vector3 size, Vector3 gap)
        {
            ...
            for (int axis = 0; axis < 3; axis++)
            {
                ...
                CustomLayout._flexItems.Clear();
                for (int i = 0; i < nodes.Count; i++)
                {
                    CustomLayout._flexItems.Add(Flex.CreateFlexItem(
                        nodes[i], axis, nodes[i].GetMeasureSize(axis, size[axis]), usedSize[axis], size[axis]));
                }

                Flex.GrowOrShrink(CustomLayout._flexItems, usedSize[axis], size[axis], gap[axis]);

                for (int i = 0; i < nodes.Count; i++)
                {
                    nodes[i].SetShrinkFillSize(axis, CustomLayout._flexItems[i].FinalSize, size[axis]);
                }
            }
        }
        ...
        public override Bounds Measure(FlexalonNode node, Vector3 size, Vector3 min, Vector3 max)
        {
            ...
            var aggregateSize = CustomLayout.AggregateLayoutSizes(node.Children, size);

            // Make sure to add the gaps between children.
            aggregateSize += this._gap * (node.Children.Count - 1);
            ...
            CustomLayout.ShrinkFillChildren(node.Children, aggregateSize, size, this._gap);
        ...
        public override void Arrange(FlexalonNode node, Vector3 layoutSize)
        {
            ...
            foreach (var child in node.Children)
            {
                ...
                nextPosition += childSize + this._gap;
            }
        }
    }
}