...
namespace Flexalon.Editor.Windows
{
    internal static class FlexalonGUI
    {
        ...
        {
            if (!FlexalonGUI._textures.TryGetValue(guid, out var texture))
            {
                var path = AssetDatabase.GUIDToAssetPath(guid);
                texture = AssetDatabase.LoadAssetAtPath<Texture2D>(path);
                FlexalonGUI._textures[guid] = texture;
            }
        ...
        {
            return FlexalonGUI.Checkbox(value, () => GUILayout.Label(label, labelStyle));
        }
        ...
        public static bool Link(string label, GUIStyle style, float lineThickness = 0.5f)
        {
            ...
            return FlexalonGUI.Link(labelContent, style, rect, lineThickness);
        }
        ...
        {
            FlexalonGUI.HorizontalLine(Color.white);
        }
        ...
        public static void HorizontalLine(Color color, float thickness = 0.5f)
        {
            ...
            FlexalonGUI.HorizontalLine(position.xMin, position.xMax, position.y, color, thickness);
        }

        public static void HorizontalLine(float startX, float endX, float y)
        {
            FlexalonGUI.HorizontalLine(startX, endX, y, Color.white);
        }
        ...
        {
            FlexalonGUI.VerticalLine(Color.white);
        }
        ...
        public static void VerticalLine(Color color, float thickness = 0.5f)
        {
            ...
            FlexalonGUI.VerticalLine(position.x, position.yMin, position.yMax, color, thickness);
        }

        public static void VerticalLine(float x, float startY, float endY)
        {
            FlexalonGUI.VerticalLine(x, startY, endY, Color.white);
        }
        ...
        public static GUIStyle CreateStyle()
        {
            ...
            style.wordWrap = true;
            style.richText = true;
            style.fontSize = FlexalonGUI.StyleFontSize;
            style.font = FlexalonGUI.StyleFont;
            return style;
        }

        public static GUIStyle CreateStyle(string textColor)
        {
            return FlexalonGUI.CreateStyle(FlexalonGUI.HexColor(textColor));
        }

        public static GUIStyle CreateStyle(Color textColor)
        {
            var style = FlexalonGUI.CreateStyle();
            FlexalonGUI.SetTextColor(style, textColor);
            return style;
        }

        public static GUIStyle CreateStyle(string textColor, string backgroundColor)
        {
            return FlexalonGUI.CreateStyle(FlexalonGUI.HexColor(textColor), FlexalonGUI.HexColor(backgroundColor));
        }

        public static GUIStyle CreateStyle(Color textColor, Color backgroundColor)
        {
            var style = FlexalonGUI.CreateStyle(textColor);
            FlexalonGUI.SetBackgroundColor(style, backgroundColor);
            return style;
        }
        ...
        public static void SetBackgroundColor(GUIStyle style, Color color)
        {
            ...
            if (!FlexalonGUI._bgTextures.TryGetValue(FlexalonGUI.StyleTag, out var textures))
            {
                textures = new List<Texture2D>();
                FlexalonGUI._bgTextures[FlexalonGUI.StyleTag] = textures;
            }
        ...
        {
            if (FlexalonGUI._bgTextures.TryGetValue(styleTag, out var textures))
            {
                ...
                FlexalonGUI._bgTextures.Remove(styleTag);
            }
        }
