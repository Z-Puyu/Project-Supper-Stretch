...
namespace CommonFrameworks.CommonUtilities.Logic {
    [Serializable]
    public abstract class AndCondition<T> : IPredicate<T> {
        [field: SerializeField, HideInInspector]
        public string Name { get; private set; } = "All of";
        
        [field: SerializeReference, ReferencePicker] 
        private List<IPredicate<T>> Predicates { get; set; } = new List<IPredicate<T>>();

        public bool Holds(T data) {
            return this.Predicates.Count == 0 || this.Predicates.TrueForAll(predicate => predicate.Holds(data));
        }
    }
}
